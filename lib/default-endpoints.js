...
function buildDefaultOpenApiPaths(config) {
  const routes = config.defaultRoutes || {};
  const prefix = (config.defaultRoutesPrefix || '/api/scalar').replace(/\/$/, '');
  const paths = {};
  ...
  if (routes.echo) {
    const echoPath = prefix + '/echo';
    if (!paths[echoPath]) paths[echoPath] = {};
    paths[echoPath]['post'] = { summary: 'Echo', description: 'Returns the received body — useful for client testing', tags: ['Dev Tools'], requestBody: { content: { 'application/json': { schema: { type: 'object' } } } }, responses: { '200': { description: 'Echoed request' } } };
  }
